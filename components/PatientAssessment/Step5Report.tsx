...
function Box({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="rounded-md border border-ink-800 bg-ink-900/60 px-2.5 py-1.5 print:border-[#d4d4d4] print:bg-[#fafaf9]">
      <div className="text-[9px] font-semibold uppercase tracking-wide text-ink-500 print:text-[#737373]">{label}</div>
      <div className="mt-0.5 whitespace-pre-wrap text-[11px] leading-relaxed text-ink-200 print:text-[#171717]">
        {children}
      </div>
    </div>
  );
...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center border-l-2 border-brand-600 pl-2 print:border-[#0f766e]">
        <span className="text-[11px] font-bold uppercase tracking-wider text-brand-400 print:text-[#0f766e]">{title}</span>
      </div>
  ...
  return (
    <div className="flex flex-col gap-3 px-4 py-3">
      ...
      <div className="print-report flex flex-col gap-4 rounded-md border border-ink-700 bg-ink-950 p-3 text-[11px] text-ink-200 print:text-[#171717]">
        ...
        <div className="border-b border-ink-800 pb-2 print:border-[#d4d4d4]">
          <div className="text-[14px] font-semibold text-ink-100 print:text-[#171717]">{draft.patientName || "Unnamed Patient"}</div>
          <div className="mt-0.5 flex flex-wrap gap-x-3 text-[10px] text-ink-400 print:text-[#737373]">
        ...
        {/* Red flag banner */}
        {redFlagsTriggered && (
          <div className="rounded-md border border-danger-600/60 bg-danger-950/40 px-3 py-2 print:border-[#dc2626] print:bg-[#fef2f2]">
            <div className="text-[11px] font-semibold text-danger-400 print:text-[#b91c1c]">⚠ Red Flags Present</div>
            <ul className="mt-1 list-inside list-disc text-[10px] text-danger-300 print:text-[#dc2626]">
        ...
        <Section title="Special Tests">
          <Box label="Joints Assessed">
            {draft.selectedJoints.length === 0 ? (
              <span className="text-ink-500">None selected.</span>
            ) : (
          ...
          {draft.selectedJoints.map((joint) => {
            ...
            return (
              <div
                ...
                className="rounded-md border border-ink-800 bg-ink-900/60 px-2.5 py-1.5 print:border-[#d4d4d4] print:bg-[#fafaf9]"
              >
                <div className="text-[9px] font-semibold uppercase tracking-wide text-ink-500 print:text-[#737373]">
                ...
                <table className="mt-1 w-full border-collapse text-[10px]">
                  <tbody>
                    {tested.map((f) => {
                      ...
                      return (
                        <tr key={f.testId} className="border-b border-ink-800/60 print:border-[#d4d4d4]">
                          <td className="py-1 pr-2 text-ink-300 print:text-[#262626]">{test?.n ?? f.testId}</td>
                          <td className="py-1 pr-2">
                            <span
                              className={`rounded px-1.5 py-0.5 text-[9px] font-semibold ${
                                notable
                                  ? "bg-danger-900/50 text-danger-300 print:bg-[#fee2e2] print:text-[#b91c1c]"
                                  : "bg-ink-800 text-ink-300 print:bg-[#f5f5f5] print:text-[#404040]"
                              }`}
                            >
                              {TEST_RESULT_LABELS[f.result]}
                            </span>
                          </td>
                          <td className="py-1 text-ink-500">{f.notes}</td>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          {Object.values(draft.testFindings).every((f) => f.result === "not_tested") && (
            <div className="text-ink-500">No tests recorded.</div>
          )}
        </Section>
        ...
        <Section title="Clinical Impression">
          {suggestions.length > 0 && (
            <div className="print:hidden flex flex-col gap-1.5 rounded-md border border-brand-700/40 bg-brand-900/10 px-2.5 py-2">
                ...
                {suggestions.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className="rounded border border-ink-700 bg-ink-800/40 px-2 py-1.5 text-left text-[11px] text-ink-200 transition hover:border-brand-600/60 hover:bg-brand-900/20"
                    >
                      + {text}
                    </button>
                  );
      ...
      <button
        ...
        className="print:hidden w-full rounded-md border border-brand-700/50 bg-brand-900/20 px-3 py-2 text-[12px] font-semibold text-brand-400 transition hover:bg-brand-900/40 disabled:cursor-wait disabled:opacity-60"
      >
      ...
      <button
        ...
        className="print:hidden w-full rounded-md border border-ink-700 px-3 py-2 text-[11px] font-medium text-ink-400 transition hover:border-danger-700/60 hover:text-danger-400"
      >
