...
export function Step5Report() {
  ...
  return (
    <div className="flex flex-col gap-3 px-4 py-3">
      ...
      <div className="print-report flex flex-col gap-4 rounded-md border border-neutral-700 bg-neutral-950 p-3 text-[11px] text-neutral-200">
        {/* Header */}
        <div className="border-b border-neutral-800 pb-2">
          <div className="text-[14px] font-semibold text-neutral-100">{draft.patientName || "Unnamed Patient"}</div>
          <div className="mt-0.5 flex flex-wrap gap-x-3 text-[10px] text-neutral-400">
        ...
        {/* Red flag banner */}
        {redFlagsTriggered && (
          <div className="rounded-md border border-red-600/60 bg-red-950/40 px-3 py-2">
            <div className="text-[11px] font-semibold text-red-400">⚠ Red Flags Present</div>
            <ul className="mt-1 list-inside list-disc text-[10px] text-red-300">
        ...
        <div className="flex flex-col gap-1.5">
          <div className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400">Subjective</div>
          {draft.chiefComplaint && (
            <div>
              <span className="font-medium text-neutral-300">Chief complaint: </span>
              {draft.chiefComplaint}
            </div>
          )}
          {draft.mechanismOfInjury && (
            <div>
              <span className="font-medium text-neutral-300">Mechanism of injury: </span>
              {draft.mechanismOfInjury}
            </div>
          )}
          {draft.duration && (
          ...
          {draft.onsetChips.length > 0 && (
            <div>
              <span className="font-medium text-neutral-300">Onset: </span>
              {draft.onsetChips.map((c) => ONSET_CHIP_LABELS[c]).join(", ")}
            </div>
          )}
          <div>
            <span className="font-medium text-neutral-300">Pain (0–10): </span>
            rest {draft.painAtRest}, worst {draft.painAtWorst}, on movement {draft.painWithMovement}
          </div>
          {draft.aggravatingFactors && (
            <div>
              <span className="font-medium text-neutral-300">Aggravating factors: </span>
              {draft.aggravatingFactors}
            </div>
          )}
          {draft.relievingFactors && (
            <div>
              <span className="font-medium text-neutral-300">Relieving factors: </span>
              {draft.relievingFactors}
            </div>
          )}
          <div>
            <span className="font-medium text-neutral-300">Night pain: </span>
        ...
        {/* Joints assessed */}
        <div className="flex flex-col gap-1.5">
          <div className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400">Joints Assessed</div>
        ...
        <div className="flex flex-col gap-2">
          <div className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400">Special Test Results</div>
          {draft.selectedJoints.map((joint) => {
            ...
            return (
              <div key={joint.categoryId} className="flex flex-col gap-1">
                <div className="text-[11px] font-medium text-neutral-300">{cat.label}</div>
                <table className="w-full border-collapse text-[10px]">
                  <tbody>
                    {tested.map((f) => {
                      ...
                      return (
                        <tr key={f.testId} className="border-b border-neutral-800/60">
                          <td className="py-1 pr-2 text-neutral-300">{test?.n ?? f.testId}</td>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="hidden print:block">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400">Clinical Impression</div>
            <div className="whitespace-pre-wrap text-neutral-200">{draft.clinicalImpression || "—"}</div>
          </div>
        </div>
      </div>
